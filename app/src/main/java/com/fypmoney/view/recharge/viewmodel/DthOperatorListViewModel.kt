...
* */
class DthOperatorListViewModel(application: Application) : BaseViewModel(application) {
    init {
        callGetOperatorList()
    }
 ...
 * */
    fun callGetOperatorList() {
        WebApiCaller.getInstance().request(
            ApiRequest(
                purpose = ApiConstant.API_GET_DTH_OPERATORS,
                endpoint = NetworkUtil.endURL(ApiConstant.API_GET_DTH_OPERATORS),
    ...
    override fun onSuccess(purpose: String, responseData: Any) {
        ...
        when (purpose) {
            ApiConstant.API_GET_DTH_OPERATORS -> {

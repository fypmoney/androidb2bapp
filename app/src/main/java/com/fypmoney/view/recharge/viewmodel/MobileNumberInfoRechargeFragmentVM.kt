...
class MobileNumberInfoRechargeFragmentVM(application: Application) : BaseViewModel(application) {
        ...
        if(!mobileNumberInfoModel?.circle.isNullOrEmpty() && operatorResponse!=null){
            if(mobileNumberInfoModel?.rechargeType==PREPAID){
                ...
                _event.value = EnterMobileNumberInfoRechargeEvent.ShowPostpaidBilScreen(
                    operatorResponse,mobileNumberInfoModel?.mobile,mobileNumberInfoModel?.operator,mobileNumberInfoModel?.circle
                )
            }
        }else{
            _event.value = EnterMobileNumberInfoRechargeEvent.ShowOperatorListScreen(mobileNumberInfoModel?.rechargeType,mobileNumberInfoModel?.mobile)
    ...
    override fun onError(purpose: String, errorResponseInfo: ErrorResponseInfo) {
        ...
        when (purpose) {
            ApiConstant.API_Explore -> {
                _state.value = EnterMobileNumberInfoRechargeState.Error(errorResponseInfo,ApiConstant.API_Explore)
            }
    ...
    sealed class EnterMobileNumberInfoRechargeState{
        ...
        data class Error(val errorResponseInfo: ErrorResponseInfo,val errorFromApi:String):EnterMobileNumberInfoRechargeState()
        data class ExploreSuccess(val explore:ArrayList<ExploreContentResponse>):EnterMobileNumberInfoRechargeState()
    }
    sealed class EnterMobileNumberInfoRechargeEvent{
        data class ShowOperatorListScreen(val rechargeType:String?,val mobileNo:String?):EnterMobileNumberInfoRechargeEvent()
        data class ShowPlanScreen(val operator:OperatorResponse?,val mobile:String?,val circle:String?):EnterMobileNumberInfoRechargeEvent()
        data class ShowPostpaidBilScreen(val operatorResponse:OperatorResponse?,val mobile:String?,val operator:String?,val circle:String?):EnterMobileNumberInfoRechargeEvent()
    }
